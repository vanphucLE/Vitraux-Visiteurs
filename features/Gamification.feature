# language: fr
Fonctionnalité: M'amuser en m'instruisant sur les vitraux

  Un utilisateur aimerait bien s'amuser tout en s'instruisant sur les vitraux.

  A partir du moment où il utilise l'application, il va pouvoir gagner des badges
  en fonction de ses visites (quand il regarde un vitrail ou fait un parcours par
  exemple).

  Dans le cas ou un utilisateur terminerait un parcours, une catégorie de badge
  de vitraux, parcours, etc...  Il obtiendrait un "haut-fait" qui serait en fait
  représenté par un badge spécial.

  Celà permettrait de rajouter un aspect de collection, afin de rendre la visite
  plus ludique pour les petits et grands. Ainsi que fidéliser les utilisateurs
  de l'application.
  ...
  Scénario: André souhaite partager son score à ses amis
    ...
    Quand il partage son score à ses amis
    Alors son score est posté sur ses réseaux sociaux préférés. ;
